...
function markVisibleMessagesAsRead() {
    if (!currentRoomId) {
        console.log('markVisibleMessagesAsRead: currentRoomId yok');
        return;
    }

    console.log('markVisibleMessagesAsRead: Başlatıldı, roomId:', currentRoomId);
    ...
    console.log('markVisibleMessagesAsRead: Bulunan mesaj elementi sayısı:', messageElements.length);
    ...
    messageElements.forEach(messageEl => {
        ...
        console.log('markVisibleMessagesAsRead: Mesaj ID:', messageId, 'Status class:', statusIcon ? statusIcon.className : 'statusIcon yok', 'Is Visible:', isVisible);
        ...
        if (statusIcon && statusIcon.classList.contains('delivered') && messageId && isVisible) {  // Sadece teslim edilmiş (✅) ve görünür olanları
            ...
            console.log('markVisibleMessagesAsRead: Okunmamış ve görünür mesaj bulundu:', messageId);
        }
    });
    ...
    if (unreadMessageIds.length > 0) {
        ...
        console.log('markVisibleMessagesAsRead: Okunmamış mesaj bulunamadı');
    }
}
...
async function markAllMessagesInRoomAsRead() {
    if (!currentRoomId) {
        console.log('markAllMessagesInRoomAsRead: currentRoomId yok');
        return;
    }

    console.log('markAllMessagesInRoomAsRead: Başlatıldı, roomId:', currentRoomId);

    try {
        // YENİ API'Yİ KULLANARAK TÜM ODADAKİ MESAJLARI OKUNDU İŞARETLE
        console.log('markAllMessagesInRoomAsRead: API çağrısı yapılıyor...');
        ...
        const data = await response.json();
        console.log('markAllMessagesInRoomAsRead: API yanıtı:', data);

        if (data.success && data.updated_count > 0) {
            console.log(`Odadaki ${data.updated_count} mesaj okundu olarak işaretlendi`);
            ...
            console.log('markAllMessagesInRoomAsRead: UI güncellemesi için bulunan element sayısı:', messageElements.length);
            ...
            console.log('markAllMessagesInRoomAsRead: API başarısız veya güncellenecek mesaj yok');
        }
    } catch (error) {
...
function updateExistingMessageStatuses() {
    if (!currentRoomId) {
        console.log('updateExistingMessageStatuses: currentRoomId yok');
        return;
    }

    console.log('updateExistingMessageStatuses: Başlatıldı');
    ...
    .then(data => {
        if (data.success && data.messages) {
            console.log('updateExistingMessageStatuses: Sunucudan gelen mesaj sayısı:', data.messages.length);
            ...
            data.messages.forEach(serverMessage => {
                ...
                if (messageElement) {
                    ...
                    if (statusIcon) {
                        ...
                        console.log(`Mesaj ${serverMessage.id}: DOM durumu: ${currentStatus}, Sunucu durumu: ${serverStatus}`);

                        // DURUM DEĞİŞMİŞSE GÜNCELLE
                        if (currentStatus !== serverStatus) {
                            console.log(`Mesaj ${serverMessage.id} durumu güncelleniyor: ${currentStatus} -> ${serverStatus}`);
